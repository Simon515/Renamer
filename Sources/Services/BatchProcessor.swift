...
final class BatchProcessor: Sendable {
    ...
    func process(folders: [URL], onProgress: @escaping @Sendable (Double, String) -> Void) async throws -> BatchResult {
        ...
        if files.isEmpty {
            return BatchResult(files: [], suggestions: [])
        }

        // Step 2: 按分类分组
        onProgress(step / totalSteps, Step.analyzing.rawValue)
        let docs = files.filter { $0.category == .document && $0.category != .image && $0.category != .video }
        let images = files.filter { $0.category == .image }
        let videos = files.filter { $0.category == .video }
        let others = files.filter { $0.category == .archive || $0.category == .application || $0.category == .other }
